...
local FenceBlock = {}
FenceBlock.__index = FenceBlock

function FenceBlock.new(buildFrame, _serviceBag: ServiceBag.ServiceBag, viewportCamera: Camera)
	local maid = Maid.new()

	local self = setmetatable({}, FenceBlock)
...
function FenceBlock:Init()
	...
	render:Subscribe(function(button)
		...
		local blockModel = ReplicatedStorage.Assets.Blocks:FindFirstChild("Fence")
		...
		self._maid:GiveTask(RunService.PreRender:Connect(function()
			if self.BuildServiceClient:GetPlacingBlockName() == "Fence" then
				tween.v = 0
				return
			end
