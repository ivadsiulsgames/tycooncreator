...
function ConveyorBlock.new(buildFrame, _serviceBag: ServiceBag.ServiceBag, viewportCamera: Camera)
	local maid = Maid.new()
...
function ConveyorBlock:Init()
	...
	render:Subscribe(function(button)
		...
		local blockModel = ReplicatedStorage.Assets.Blocks:FindFirstChild("Conveyor")
		...
		self._maid:GiveTask(RunService.PreRender:Connect(function()
			if self.BuildServiceClient:GetPlacingBlockName() == "Conveyor" then
				tween.v = 0
				return
			end
