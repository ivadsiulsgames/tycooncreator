...
local MATRIX_ROTATION = CFrame.new(0, 0, 0, 0, 0, 1, 0, 1, 0, -1, 0, 0)
local CORNERS = {
	Vector3.new(1, 1, 1),
	Vector3.new(1, 1, -1),
	Vector3.new(-1, 1, 1),
...
local function getClosestCorner(position: Vector3, part: BasePart): (Vector3, Vector3, Vector3)
	...
	for _, corner in pairs(CORNERS) do
		local worldPosition = part.CFrame
			* (
				Vector3.new(corner.X * (part.Size.X / 2), corner.Y * (part.Size.Y / 2), corner.Z * (part.Size.Z / 2))
